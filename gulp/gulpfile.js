...
const ejsCompile = (done) => {
  const pageDataFile = "../src/ejs/pageData/pageData.json";
  const productDataFile = "../src/ejs/pageData/productData.json";

  let pageData = {};
  let productData = {};
  
  // pageData.jsonを読み込み（エラーハンドリング付き）
  try {
    pageData = JSON.parse(fs.readFileSync(pageDataFile, "utf8"));
  } catch (error) {
    console.warn("pageData.json not found or invalid, using empty object");
  }
  
  // productData.jsonを読み込み（エラーハンドリング付き）
  try {
    productData = JSON.parse(fs.readFileSync(productDataFile, "utf8"));
  } catch (error) {
    console.warn("productData.json not found or invalid, using empty object");
  }
